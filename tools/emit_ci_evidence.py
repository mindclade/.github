...
def checks_from_args(args: argparse.Namespace) -> list[Any]:
    ...
    for report_path in _report_paths(args.report_paths):
        ...
        if report.parent.name == "reports" and ARTIFACT_REPORT_PATH.fullmatch(f"reports/{report.name}"):
...
def _report_root(candidate: Path) -> Path | None:
    return next((resolved for lexical, resolved in _report_roots() if candidate == lexical or lexical in candidate.parents), None)
...
def _read_report_snapshot(path: Path) -> bytes:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"report path must not contain symlinks and must be safely readable: {path}") from exc
        try:
...
def stage_reports(value: str | None, staging_directory: str | Path) -> list[Path]:
    ...
    for index, source_value in enumerate(source_paths, start=1):
        ...
        if aggregate > MAX_REPORT_AGGREGATE_BYTES:
            raise ValueError(f"reports exceed the {MAX_REPORT_AGGREGATE_BYTES}-byte aggregate limit")
...
def verify_artifact_directory(
    ...
    *,
    ...
    artifact = Path(os.path.abspath(artifact_directory))
    ...
    for check in checks:
        ...
        if isinstance(report_size, bool) or not isinstance(report_size, int) or not 0 <= report_size <= MAX_REPORT_BYTES:
        ...
        with os.scandir(reports_directory) as entries:
            for entry in entries:
                ...
                try:
                    ...
                    raise ValueError(f"artifact report cannot be inspected safely: {report_path}") from exc
                if entry.is_symlink() or not stat.S_ISREG(entry_status.st_mode):
                    raise ValueError(f"artifact report must be a non-symlink regular file: {report_path}")
...
class BuildkiteClient:
    ...
    def verify_evidence(self, pipeline: str, build_id: str, build_number: str, expected_commit: str, timeout_seconds: int) -> tuple[dict[str, Any], bytes | None]:
        build_id = _canonical_build_id(build_id)
        build_number = _canonical_build_number(build_number)
        expected_commit = _canonical_source_revision(expected_commit)
...
def command_emit(args: argparse.Namespace) -> int:
    ...
    write_output(args.github_output, {"artifact_name": args.artifact_name, "artifact_path": artifact_path or str(args.output), "evidence_json": evidence_json, "evidence_path": str(args.output), "evidence_digest": evidence_digest, "correlation_id": evidence["correlation_id"], "conclusion": evidence["conclusion"], "reason_code": evidence["reason_code"]})
    print(canonical_json({"ok": True, "artifact_name": args.artifact_name, "artifact_path": artifact_path or str(args.output), "evidence_digest": evidence_digest, "evidence_path": str(args.output)}))
    return 0
