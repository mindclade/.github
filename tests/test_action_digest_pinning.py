...
class ActionDigestPinningTest(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            subprocess.run(
                [
                    "git",
                    "-c",
                    "user.name=Mindclade Test",
                    "-c",
                    "user.email=test@mindclade.dev",
                    "commit",
                    "-qm",
                    "implementation",
                ],
            ...
            template.write_text(
                "jobs:\n  call:\n    uses: mindclade/.github/.github/workflows/reusable-example.yml@"
                + implementation
                + "\n",
                encoding="utf-8",
            )
            ...
            template.write_text(
                "jobs:\n  call:\n    uses: mindclade/.github/.github/workflows/missing.yml@"
                + implementation
                + "\n",
                encoding="utf-8",
            )
    ...
    def test_internal_reusable_workflow_requires_a_full_sha(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            workflow.parent.mkdir(parents=True)
            workflow.write_text(
                "jobs:\n  call:\n    uses: mindclade/.github/.github/workflows/reusable-metadata-validation.yml@"
                + SHA
                + "\n",
                encoding="utf-8",
            )
    ...
    def test_container_reference_requires_the_exact_approved_digest(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertFalse(validator.validate_pins(root)["ok"])
            workflow.write_text(
                "jobs:\n  scan:\n    container: ghcr.io/example/scorecard@sha256:"
                + "a" * 64
                + "\n",
                encoding="utf-8",
            )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for script in scripts:
                with self.subTest(script=script):
                    ...
                    self.assertFalse(outcome["ok"])
                    self.assertTrue(
                        any("approved Scorecard script" in error for error in outcome["errors"])
                    )
    ...
    def test_buildkite_client_uses_fixed_api_and_injectable_transport(self) -> None:
            ...
            return _Response(
                {
                    ...
                    "commit": SHA,
                    "state": "scheduled",
                    "web_url": "https://buildkite.example/build-id",
                }
            )
        ...
        self.assertEqual(
            "https://api.buildkite.com/v2/organizations/mindclade/pipelines/validate/builds",
            observed["url"],
        )
        self.assertEqual(15, observed["timeout"])
        self.assertIs(
            evidence.BuildkiteClient("token-not-logged", "mindclade")._artifact_opener,
            evidence._open_without_redirects,
        )
    ...
    def test_buildkite_dispatch_requires_the_exact_response_commit(self) -> None:
        for response_commit in (None, "b" * 40, SHA.upper()):
            ...
            def opener(
                _request: object,
                timeout: int,
                response_commit: str | None = response_commit,
            ) -> _Response:
                self._assert_request_timeout(timeout)
                return _Response(
                    {"id": BUILD_ID, "number": 7, "commit": response_commit, "state": "scheduled"}
                )
        ...
        def opener(_request: object, timeout: int) -> _Response:
            ...
            if calls == 1:
                raise urllib.error.HTTPError(
                    "https://api.buildkite.com", 429, "slow down", {"Retry-After": "2"}, None
                )
            if calls == 2:
                raise urllib.error.URLError("temporary transport failure")
            if calls == 3:
                raise urllib.error.HTTPError(
                    "https://api.buildkite.com", 503, "unavailable", {}, None
                )
            return _Response(
                {"id": BUILD_ID, "number": 7, "commit": SHA, "state": "passed", "blocked": False}
            )
    ...
    def test_buildkite_mutating_requests_are_never_retried(self) -> None:
        ...
        client = evidence.BuildkiteClient(
            "token-not-logged", "mindclade", opener, sleeper=lambda _: self.fail("slept")
        )
    ...
    def test_buildkite_rechecks_commit_on_every_poll(self) -> None:
        ...
        client = evidence.BuildkiteClient(
            "token-not-logged", "mindclade", opener, sleeper=lambda _: None
        )
    ...
    def test_required_check_never_interpolates_caller_inputs_in_a_shell_script(self) -> None:
        workflow = (
            Path(__file__).resolve().parents[1] / ".github/workflows/reusable-required-check.yml"
        ).read_text(encoding="utf-8")
        ...
        self.assertIn(
            "EXPECTED_PIPELINE_DEFINITION_REVISION: ${{ inputs.pipeline_definition_revision }}",
            cancel_step,
        )
        self.assertIn(
            '--expected-pipeline-definition-revision "${EXPECTED_PIPELINE_DEFINITION_REVISION}"',
            cancel_step,
        )
    ...
    def test_buildkite_identifiers_are_validated_before_transport(self) -> None:
        ...
        client = evidence.BuildkiteClient("token-not-logged", "mindclade", opener)
        for build_id, build_number in (
            ("$(exfiltrate-token)", BUILD_NUMBER),
            (BUILD_ID, "07"),
            ("AAAAAAAA-AAAA-4AAA-8AAA-AAAAAAAAAAAA", BUILD_NUMBER),
        ):
            with (
                self.subTest(build_id=build_id, build_number=build_number),
    ...
    def test_buildkite_verify_uses_numbered_routes_and_strips_redirect_authorization(self) -> None:
        ...
        evidence_document = {
            ...
            "checks": [
                {"name": "required", "conclusion": "PASS", "report_digest": "sha256:" + "f" * 64}
            ],
                ...
                return _Response(
                    {
                        ...
                        "number": 7,
                        "commit": SHA,
                        "state": "running" if build_calls == 1 else "passed",
                        "blocked": False,
                        "web_url": "https://buildkite.example/build-id",
                    }
                )
        ...
        self.assertEqual(
            [COMPACT_BUILD_URL, COMPACT_BUILD_URL], [url for url, _, _ in api_calls[:2]]
        )
        self.assertTrue(
            all(url.startswith("https://api.buildkite.com/v2/") for url, _, _ in api_calls)
        )
        self.assertTrue(
            all(authorization == "Bearer token-not-logged" for _, _, authorization in api_calls)
        )
    ...
    def test_buildkite_response_binding_mismatches_stop_before_artifact_access(self) -> None:
        for returned_id, returned_number in ((OTHER_BUILD_ID, 7), (BUILD_ID, 8)):
            ...
            def opener(
                ...
                returned_number: int = returned_number,
            ) -> _Response:
                nonlocal calls
                self._assert_request_timeout(timeout)
                calls += 1
    ...
    def test_buildkite_blocked_flag_is_required_and_prevents_evidence_acceptance(self) -> None:
        ...
        def blocked_opener(request: object, timeout: int) -> _Response:
            ...
            return _Response(
                {"id": BUILD_ID, "number": 7, "commit": SHA, "state": "passed", "blocked": True}
            )
    ...
    def test_buildkite_cancel_uses_numbered_route_and_validates_response_bindings(self) -> None:
        ...
        self.assertEqual(
            [
                (
                    "https://api.buildkite.com/v2/organizations/mindclade/pipelines/validate/builds/7/cancel",
                    "PUT",
                )
            ],
            calls,
    ...
    def test_buildkite_rejects_unfinished_or_misdirected_evidence_artifacts(self) -> None:
            ...
            (
                {
                    ...
                    "filename": "ci-evidence.json",
                    "state": "finished",
                    "download_url": "https://attacker.example/evidence",
                },
                "download_url mismatch",
            ),
        )
        for artifact, error in cases:
            ...
            def opener(
                request: object,
                timeout: int,
                artifact: dict[str, object] = artifact,
            ) -> _Response:
                nonlocal calls
                self._assert_request_timeout(timeout)
                calls += 1
    ...
    def test_buildkite_scans_all_artifact_pages_without_leaking_authorization(self) -> None:
        ...
        def opener(request: object, timeout: int) -> _Response:
            ...
            if url.endswith("/artifacts"):
                return _Response(
                    [artifact(ARTIFACT_ID)], headers={"Link": f'<{next_url}>; rel="next"'}
                )
            if url == next_url:
    ...
    def test_buildkite_rejects_invalid_artifact_redirects_without_storage_access(self) -> None:
        cases = (
            (
                ...
                {"Location": "https://storage.example/evidence"},
                "did not return HTTP 302",
            ),
            (
                302,
                {"url": "https://storage.example/one"},
                {"Location": "https://storage.example/two"},
                "redirect URL mismatch",
            ),
            (
                302,
                {"url": "http://storage.example/evidence"},
                {"Location": "http://storage.example/evidence"},
        ...
        for status, payload, headers, error in cases:
            ...
            with self.subTest(error=error):
                client = evidence.BuildkiteClient(
                    "token-not-logged", "mindclade", opener, artifact_opener=artifact_opener
                )
                with self.assertRaisesRegex(RuntimeError, error):
    ...
    def test_buildkite_retries_artifact_redirect_and_signed_storage_gets_without_auth_leak(
        self,
    ) -> None:
        signed_url = "https://artifacts.example.test/evidence.json?signature=secret"
        redirect_calls = 0
        ...
        def artifact_opener(request: object, timeout: float) -> _Response:
            ...
            if storage_calls == 1:
                raise urllib.error.HTTPError(
                    request.full_url, 429, "slow down", {"Retry-After": "1"}, None
    ...
    def test_buildkite_evidence_requires_buildkite_producer_and_all_pass_checks(self) -> None:
        ...
        try:
            ...
            self.assertEqual(
                [],
                evidence.buildkite_evidence_binding_errors(
                    document, {"id": BUILD_ID, "number": 7, "commit": "e" * 40}, args
                ),
            )
            document["producer"] = "github_actions"
            document["checks"][0]["conclusion"] = "FAIL"
            errors = evidence.buildkite_evidence_binding_errors(
                document, {"id": BUILD_ID, "number": 7, "commit": SHA}, args
            )
        finally:
    ...
    def test_mismatched_evidence_is_rejected_by_schema(self) -> None:
        ...
        errors = evidence.validate_schema(
            {"source_revision": "stale", "context_digest": "mismatch"}, schema
        )
    ...
    def test_compromise_drill_rejects_supply_chain_token_and_shell_escalation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue(
                any("contents must be read" in error for error in permission_errors),
                permission_errors,
            )
            self.assertTrue(
                any("id-token" in error for error in permission_errors), permission_errors
            )
            self.assertTrue(
                any("through the environment" in error for error in workflow_errors),
                workflow_errors,
            )
            self.assertTrue(
                any("bounded concurrency" in error for error in workflow_errors), workflow_errors
            )
